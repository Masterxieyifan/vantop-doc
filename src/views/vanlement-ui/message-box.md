...
<template>
  ...
  <el-button type="text" @click="open1">点击打开 默认 title Message Box</el-button>
</template>
...
:::demo 调用`$confirm`方法即可打开消息提示，它模拟了系统的 `confirm`。Message Box 组件也拥有极高的定制性，我们可以传入`options`作为第三个参数，它是一个字面量对象。`type`字段表明消息类型，可以为`danger`，和`warning`，无效的设置将会被忽略。注意，第二个参数`title`必须定义为`String`类型，如果是`Object`，会被理解为`options`。在这里我们用了 Promise 来处理后续响应。

```html
<template>
  <el-button type="text" @click="openDanger">点击打开 Danger Message Box</el-button>
  <el-button type="text" @click="openWarning">点击打开 Warning Message Box</el-button>
</template>
...
<script>
  export default {
    methods: {
      openDanger() {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '确认删除数据？', {
          type: 'danger',
        }).then(() => {
          this.$message({
            type: 'success',
      ...
      openWarning() {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '确认消息', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        }).then(() => {
...
| type | 消息类型，用于显示图标 | string | warning / danger | — |
